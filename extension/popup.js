(function () {
  ...
  function formatDate(isoOrTs) {
    try {
      var d =
        typeof isoOrTs === "number" ? new Date(isoOrTs) : new Date(isoOrTs);
  ...
  function renderSavedList(versions, apiBase) {
    ...
    versions.forEach(function (v) {
      ...
      li.title = "点击在新标签页中查看";
      li.innerHTML =
        '<div class="version-meta">v' +
        (v.version_num || v.id) +
        " · " +
        formatDate(v.created_at) +
        "</div>" +
        '<div class="version-thumb">' +
        escapeHtml(thumbnail(v.content)) +
        "</div>";
  ...
  function loadAll(url) {
    if (
      ...
      document.getElementById("memory-empty").textContent =
        "当前页面无法获取 URL";
  ...
  document
    .getElementById("btn-refresh-saved")
            ...
            getApiBaseUrl()
              .then(function (apiBase) {
                ...
                return fetch(saveUrl, {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
                  body: JSON.stringify({ name: tab.url, content: content }),
                });
              })
              .then(function (res) {
                if (!res.ok) throw new Error(res.statusText || "保存失败");
